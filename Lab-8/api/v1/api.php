...
try {
  ...
  if ( 'address' === $resource ) {
    ...
    $usr = $serverData['email'];
    $pwd = $serverData['pass'];
    
    // Check if the login is correct, if it is, set the JWT.
    if($util->login($usr, $pwd)) {
      $restServer->setData(array("token" => $jwt->generateJWT(array("email"=>$usr), $secrect_key)));
    }
    else {
      $restServer->setErrors("E-mail or password is incorrect.");
    }
