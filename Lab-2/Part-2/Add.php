  ...
  <body>
    ...
    <div class="container-fluid">
      ...
      switch($_SERVER['REQUEST_METHOD']) {
        case "POST":
          if(empty($email) || !$v->ValidateEmail($email)) {
            if(empty($email)) array_push($errors, "Email is required");
            if(!$v->ValidateEmail($email) && !empty($email)) array_push($errors, "Email is invalid");    
          }

          if(empty($zip) || !$v->ValidateZip($zip)) {
            if(empty($zip)) array_push($errors, "Zipcode is required");
            if(!$v->ValidateZip($zip) && !empty($zip)) array_push($errors, "Zipcode is invalid");
          }

          if(empty($birthday) || $v->ValidateDate($birthday)) {
            if(empty($birthday)) array_push($errors, "Birthdate is required");
            if(!$v->ValidateDate($birthday) && !empty($birthday)) array_push($errors, "Birthdate is invalid");
          }
          ...
          {
            // Insert new data.
            $CRUD->create($fullname, $email, $addressline1, $city, $state, $zip, $birthday);
